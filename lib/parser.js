...
export function getMarkdownParagraphText(paragraph) {
    ...
    for (const span of paragraph.spans) {
        parts.push(getMarkdownSpanText(span));
    }
    return parts.join('');
}
...
function getMarkdownSpanText(span) {
    if ('image' in span) {
        ...
        const parts = [];
        for (const linkSpan of span.link.spans) {
            parts.push(getMarkdownSpanText(linkSpan));
        }
        return parts.join('');
    } else if ('style' in span) {
        const parts = [];
        for (const styleSpan of span.style.spans) {
            parts.push(getMarkdownSpanText(styleSpan));
        }
        return parts.join('');
    }
    return span.text ?? '';
...
const rTable = /^ {0,3}(?::?-+:?\s*)?(?:\|\s*:?-+:?\s*)+(?:\|\s*)?$/;
const rTableRow = /^ {0,3}(?:(?:\\\||[^|])+\s*)?(?:\|\s*(?:\\\||[^|])*?\s*)+(?:\|\s*)?/;
...
function parseMarkdownInternal(markdown, startLineNumber, linkRefsRaw) {
    ...
    const closeParagraph = (paragraphStyle = null) => {
        ...
        // Ordinary (or header) paragraph...
            ...
            while (matchLinkDef !== null) {
                ...
                if (!Object.hasOwn(linkRefs.defs, linkRefKey)) {
    ...
    for (const markdownString of markdownStrings) {
        ...
        for (const line of markdownString.replaceAll('\t', '    ').split(rLineSplit)) {
            const lineIndent = line.match(rIndent)[0].length;
            emptyLinePrev = emptyLine;
            emptyLine = lineIndent === line.length;
            lineNumber += 1;
            ...
            if (tablePart !== null) {
                ...
                const matchTableHeader =
                    (tableHeader !== null && line.match(rTable) !== null ? tableHeader.match(rTableRow) : null);
    ...
    if (linkRefsRaw === null) {
        for (const linkRef of linkRefs.links) {
            if (Object.hasOwn(linkRefs.defs, linkRef.refKey)) {
...
function paragraphSpans(text, linkRefs) {
    ...
    for (const match of text.matchAll(rSpans)) {
        ...
        const [matchFullText] = match;

        // Add any preceding text
        if (ixSearch < match.index) {
            spans.push({'text': removeEscapes(text.slice(ixSearch, match.index))});
        }
        ixSearch = match.index + matchFullText.length;

        // Code?
        if (matchGroups.code) {
            const {code, codeT} = matchGroups;
            const codeTLen = codeT.length;
            const codeText = code.slice(codeTLen, code.length - codeTLen);
            let codeScrubbed = codeText.replaceAll(rCodeNewlines, ' ');
            const codeMatch = rCodeSpaces.exec(codeScrubbed);
            if (codeMatch !== null) {
                const [, codeGroup] = codeMatch;
                codeScrubbed = codeGroup;
            }
        ...
        const boldStr = matchGroups.bold || matchGroups.boldu;
        if (boldStr) {
            const boldText = boldStr.slice(2, boldStr.length - 2);
            spans.push({'style': {'style': 'bold', 'spans': paragraphSpans(boldText, linkRefs)}});
            continue;
        }

        // Italic style? (italic or italicu)
        const italicStr = matchGroups.italic || matchGroups.italicu;
        if (italicStr) {
            const italicText = italicStr.slice(1, italicStr.length - 1);
            spans.push({'style': {'style': 'italic', 'spans': paragraphSpans(italicText, linkRefs)}});
            continue;
        }

        // Link reference?
        if (matchGroups.linkRef) {
            const {linkRefText, linkRefOtherText = null} = matchGroups;
            if (matchGroups.linkRef.startsWith('!')) {
                spans.push(createImageRefSpan(linkRefText, linkRefOtherText, matchFullText, linkRefs));
            } else {
                spans.push(createLinkRefSpan(linkRefText, linkRefOtherText, matchFullText, linkRefs));
            }
            continue;
        }

        // Strikethrough style?
        if (matchGroups.strike) {
        ...
        if (matchGroups.br || matchGroups.brHtml) {
            spans.push({'br': 1});
            continue;
        }

        // Link with inline image?
        if (matchGroups.linkImg) {
        ...
        if (matchGroups.linkRefImg) {
            ...
            spans.push(createLinkRefSpan(linkRefImgLinkText, [imgSpan], matchFullText, linkRefs, true));
            continue;
        }

        // Link reference with inline image reference
        if (matchGroups.linkRefImgRef) {
            ...
            spans.push(createLinkRefSpan(linkRefImgRefLinkText, [imgSpan], matchFullText, linkRefs, true));
        }
    }
...
function getLinkText(matchGroups, prefix) {
    ...
    if (title !== null) {
        title = removeEscapes(title.slice(1, title.length - 1), true);
    }
...
// Remove span text escapes and replace character references. This is a single left-to-right pass -
// an escape or a character reference is consumed as a unit and its replacement is never re-scanned,
// so an escape hides a character reference and a replacement is never replaced again by a later pass.
function removeEscapes(text, href = false) {
    // No character references? Removing the escapes is then a single substitution.
    if (!text.includes('&')) {
        if (!text.includes('\\')) {
            return text;
        }
    ...
    return text.replace(href ? rUnescapeHref : rUnescape, (match, escaped, name, decimal, hex) => {
        // Escape?
        if (escaped !== undefined) {
            return escaped;
        }

        // Entity reference? An unknown entity name is left as-is.
        if (name !== undefined) {
            return entityRefs[name] ?? match;
        }

        // Decimal or hexadecimal numeric character reference
        return String.fromCharCode(decimal !== undefined ? parseInt(decimal, 10) : parseInt(hex, 16));
    });
}
...
const rEscapeHref = /\\([!-/:-@[-`{-~])/g;
const rUnescape =
    /\\(?<escaped>[!-~])|&(?<name>[A-Za-z]+[0-9]*);|&#(?<decimal>[0-9]{1,7});|&#[Xx](?<hex>[A-Fa-f0-9]{1,6});/g;
const rUnescapeHref =
    /\\(?<escaped>[!-/:-@[-`{-~])|&(?<name>[A-Za-z]+[0-9]*);|&#(?<decimal>[0-9]{1,7});|&#[Xx](?<hex>[A-Fa-f0-9]{1,6});/g;
const entityRefs = {
    'acute': '\xb4',
