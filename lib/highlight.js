...
export function codeBlockElements(codeBlock, options) {
    ...
    if (codeBlockLanguage) {
        ...
        if (options) {
            const {codeBlocks} = options;
            const renderFn = codeBlocks ? codeBlocks[language] : null;
            if (renderFn) {
        ...
        const renderFn = highlightBuiltin[language];
        if (renderFn) {
            return renderFn(codeBlock, options);
        }
    }
...
const highlightMemberNames = ['builtin', 'comment', 'keyword', 'literal', 'preprocessor', 'string', 'tag'];
...
export function compileHighlightModels(highlights) {
    ...
    for (const highlight of highlights) {
        ...
        const namedParts = [];
        for (const memberName of highlightMemberNames) {
            const memberPatterns = highlight[memberName];
            if (memberPatterns) {
                const partStrs = [];
                for (const memberPattern of memberPatterns) {
                    partStrs.push(`(?:${memberPattern})`);
                }
                namedParts.push(`(?<${memberName}>${partStrs.join('|')})`);
            }
        }

        // Compile and register the language's render function
        if (namedParts.length) {
            const highlightRegex = new RegExp(namedParts.join('|'), 'gm');
            const highlightFn = (codeBlock, options) => highlightRenderFn(highlightRegex, codeBlock, options);
...
function highlightRenderFn(highlightRegex, codeBlock, options) {
    return highlightElements(highlightRegex, codeBlock.lines, options);
}
...
function highlightElements(highlightRegex, lines, options = null) {
    // Join the text lines
    const text = lines.map((line) => (line.endsWith('\n') ? line : `${line}\n`)).join('');

    // The copy button element
    let preAttr = null;
    let codeAttr = null;
    let copyElements = null;
    if (options && options.copyLinks) {
        // Increment the copy links ID index
        const copyLinksIndex = options.copyLinksIndex ?? 1;
        options.copyLinksIndex = copyLinksIndex + 1;

        // Generate the code block ID and copy button element
        const preID = `__markdown_copy_${copyLinksIndex}`;
    ...
    if (!highlightRegex) {
        ...
        codeChildren = [];
        let ixSearch = 0;
        for (const mHighlight of text.matchAll(highlightRegex)) {
            const matchGroups = mHighlight.groups;
            const [highlightText] = mHighlight;
            const highlightIndex = mHighlight.index;

            // Add the pre-text span
            if (ixSearch < highlightIndex) {
                codeChildren.push({'text': text.slice(ixSearch, highlightIndex)});
            }

            // Determine the highlight style from which named group matched - exactly one group
            // matches, so the members are tested most-frequent first and "tag" needs no test
            let attrStyle;
            if (matchGroups.string) {
                attrStyle = 'color: var(--markdown-model-color-highlight-string);';
            } else if (matchGroups.keyword) {
                attrStyle = 'color: var(--markdown-model-color-highlight-keyword);';
            } else if (matchGroups.literal) {
                attrStyle = 'color: var(--markdown-model-color-highlight-literal);';
            } else if (matchGroups.comment) {
                attrStyle = 'color: var(--markdown-model-color-highlight-comment);';
            } else if (matchGroups.builtin) {
                attrStyle = 'color: var(--markdown-model-color-highlight-builtin);';
            } else if (matchGroups.preprocessor) {
                attrStyle = 'color: var(--markdown-model-color-highlight-preprocessor);';
            } else {
                attrStyle = 'color: var(--markdown-model-color-highlight-tag);';
            }
            ...
            ixSearch = highlightIndex + highlightText.length;
        }

        // Add the final un-matched text
        if (ixSearch < text.length) {
            codeChildren.push({'text': text.slice(ixSearch)});
        }
    }
...
function createWordListRegex(...words) {
    ...
    for (const word of words) {
        escapedParts.push(escapeRegexText(word));
    }
    return `\\b(?:${escapedParts.join('|')})\\b`;
}
